...
export default class SafeAdd extends Component {
    state = {
    ...
    addData = (param) => {
        console.log(param);
        post(`${mobileHttp}${CONSTRUCTION_URL}/check/safety/app/issue/add`, param).then(data => {
            if (data.code !== 200) {
    ...
    getCheckData = () => {
        const {projectId} = this.state;
        post(`${http}/bim6d-construction/check/safety/rules/list`, {projectId, modelId, keyword: ''}).then(data => {
            if (data.code == "200") {
    ...
    setNormal = isNormal => {
        this.setState({safetyLevel: isNormal ? '0' : null})
    }

    setVip = isVip => {
        this.setState({safetyLevel: isVip ? '1' : null})
    }
    ...
    getPersonRow = checkRow => {
        this.setState({
            safetyCheckIssuePrincipalList: checkRow.map(c => {
                let o = {};
                o.principal = c.id;
    ...
    addQuality = () => {
        const {checkAreaId,projectId, modelId, floorId, floorName, specialtyId, specialtyName, isPublic, isNotify, safetyLevel,deductMarks, remark, principal, safetyCheckIssuePrincipalList, ruleIds, fileIds} = this.state;
        // if (!floorId) {
        //     Toast.info('请选择楼层');
        //     return;
        // }
    ...
    cancel = () => {
        this.props.navigation.navigate('SafeList');
    }
    ...
    renderArea=()=>{
        ...
        if (areaData.length) {
            return areaData.map(p => (<RadioItem
                checked={this.state.checkAreaId === p.id}
                key={p.id}
                onChange={event => {
                    if (event.target.checked) {
                        this.setState({checkAreaId: p.id, areaName: p.name});
                    }
                }}
            >
                {p.name}
        ...
        return (<ScrollView style={styles.wrapper}>
            ...
            <View style={styles.item}>
                ...
                <View style={styles.listContent}>
                    <Tag
                        style={styles.tag}
                        selected={this.state.safetyLevel == '1'}
                        onChange={this.setVip}>重大隐患</Tag>
                    <Tag
                        style={styles.tag}
                        selected={this.state.safetyLevel == '0'}
            ...
            <View style={styles.item}>
                ...
                <View style={styles.inputWrapper}>
                    <InputItem
                        last
                        style={styles.input}
                        value={this.state.deductMarks}
                        onChange={val => this.setState({deductMarks: val})}
                    />
                </View>
            </View>
            ...
            <Modal
                ...
                maskClosable
            >
                <ScrollView style={{paddingVertical: 20, height: 200}}>
                    <TreeSelect
                        multiple
