...
import {Card, WhiteSpace, WingBlank, Drawer, Icon, Modal} from '@ant-design/react-native';
import DrawerList from './DrawerList'
import QualityAdd from './QualityAdd'
import {post} from "../../config/fetch";
import {mobileHttp, http, APP_URL, SYSTEM_URL} from "../../config/http";
...
class QualityList extends Component {
    ...
    getData = () => {
        ...
        post(`${mobileHttp}${APP_URL}/check/quality/app/issue/list`, queryData).then(data => {
            if (data.code != 200)
                return;
    ...
    pressItem = item => {
        this.props.navigation.navigate('QualityDetail', {name: item.modelName, id: item.id});
    }

    addQues = () => {
        this.props.navigation.navigate('QualityAdd');
    }
    ...
    render() {
        ...
        const sidebar = (
            <ScrollView>
                <DrawerList onQuery={this.getQuery}/>
            </ScrollView>
        );
...
export default withNavigationFocus(QualityList)
