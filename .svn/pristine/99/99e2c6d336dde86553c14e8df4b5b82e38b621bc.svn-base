...
import {TextareaItem, InputItem, Button, Tag, Toast} from '@ant-design/react-native';
import {post} from "../../config/fetch";
import {mobileHttp,http,SYSTEM_URL, APP_URL} from "../../config/http";
...
export default class SafeDetail extends Component {

    static navigationOptions = ({navigation}) => {
        const {params} = navigation.state;

        return {
            title: params ? params.name : '安全详情',
    ...
    getData = () => {
        ...
        const safeId = this.props.navigation.state.params.id;
        post(`${mobileHttp}${APP_URL}/check/safety/app/issue/get/${safeId}`, {}).then(data => {
            if (data.code != 200)
                return;
    ...
    changeStatusData = () => {
        ...
        post(`${mobileHttp}${APP_URL}/check/safety/app/issue/updateStatus/${safeId}/${status}`, {}).then(data => {
            if (data.code != 200) {
    ...
    changeRectificationData = () => {
        ...
        post(`${mobileHttp}${APP_URL}/check/safety/app/issue/updateRectificationRemake`, {id:safeId,rectificationRemake}).then(data => {
            if (data.code != 200) {
                Toast.fail(data.message)
                return;
            }
    ...
    getImageData = fileId => {
        post(`${http}${SYSTEM_URL}/file/genShareLink/${fileId}`, {})
            .then(data => {
                if (data.code != 200)
                    return null;
                this.setState(state => ({uriAry: [...state.uriAry, data.data]}))
            })
    }
    ...
    render() {
        ...
        return (<ScrollView style={styles.wrapper}>
            ...
            <View style={styles.item}>
                ...
                <View style={styles.listContent}>
                    <View style={styles.itemContent}>
                        <Text>{qualityDetail.areaName}</Text>
                    </View>
                </View>
            </View>
            {/*楼层*/}
            <View style={styles.item}>
                ...
                <View style={styles.listContent}>
                    <View style={styles.itemContent}>
                        <Text>{qualityDetail.floorNames}</Text>
                    </View>
                </View>
            </View>
            ...
            <View style={styles.item}>
                ...
                <View style={styles.listContent}>
                    <View style={styles.itemContent}>
                        <Text>{this.renderLevel(qualityDetail.safetyLevel)}</Text>
                    </View>
                </View>
            </View>
            {/*是否公开*/}
            ...
            <View style={styles.item}>
                ...
                <View style={styles.listContent}>
                    <View style={styles.itemContent}>
                        <Text>{qualityDetail.deductMarks}</Text>
                    </View>
                </View>
            </View>
            ...
            <View style={styles.item}>
                ...
                <View style={styles.listContent}>
                    <View style={styles.itemContent}>
                        {this.renderCheckList(qualityDetail.safetyRulesContentList)}
                    </View>
                </View>
            </View>
            ...
            <View style={styles.item}>
                ...
                <View style={styles.listContent}>
                    <View style={styles.remarkWrapper}>
                        ...
                        <Button style={styles.remarkBtn} type='ghost' onPress={this.setRectificationRemake}>保存</Button>
                    </View>
                </View>
            </View>
